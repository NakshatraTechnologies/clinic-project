  ...
  return (
    <div>
      ...
      <div className="d-flex justify-content-between align-items-start mb-4 flex-wrap gap-2">
        <div>
          <h3 style={{ fontWeight: 800 }}>
            <span className="material-symbols-outlined me-2" style={{ color: '#0d9488', fontSize: '28px', verticalAlign: 'middle' }}>support_agent</span>
            Support Tickets
          </h3>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', margin: 0 }}>Raise and track your support requests.</p>
        </div>
        ...
        {statusTabs.map(tab => (
          <button
            ...
            style={{
              ...
              background: statusFilter === tab.value ? '#0d9488' : '#f0f2f4',
      ...
      {loading ? (
        ...
        <div className="card p-5 text-center" style={{ border: '2px dashed var(--border)' }}>
          ...
          <p style={{ color: 'var(--text-muted)' }}>You haven't raised any support tickets yet.</p>
        </div>
      ) : (
              ...
              <tbody>
                {tickets.map(t => (
                  <tr key={t._id} style={{ cursor: 'pointer' }} onClick={() => openDetail(t._id)}>
                    <td style={{ fontWeight: 700, padding: '0.875rem 1rem', color: '#0d9488' }}>{t.ticketNumber}</td>
                    <td style={{ fontWeight: 600, maxWidth: '220px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.subject}</td>
                    <td><span style={{ textTransform: 'capitalize' }}>{t.category}</span></td>
                    <td>
                      <span style={{ ...PRIORITY_COLORS[t.priority], padding: '0.25rem 0.625rem', borderRadius: '6px', fontWeight: 600, fontSize: '0.75rem', textTransform: 'uppercase' }}>{t.priority}</span>
                    </td>
                    <td>
                      <span style={{ ...STATUS_COLORS[t.status], padding: '0.25rem 0.625rem', borderRadius: '6px', fontWeight: 600, fontSize: '0.75rem', textTransform: 'uppercase' }}>{t.status?.replace('_', ' ')}</span>
                    </td>
            ...
            <div className="modal-content" style={{ borderRadius: '16px', border: 'none' }}>
              <div className="modal-header" style={{ borderBottom: '1px solid var(--border)', padding: '1.25rem 1.5rem' }}>
                <h5 className="modal-title" style={{ fontWeight: 800 }}>
                  <span className="material-symbols-outlined me-2" style={{ verticalAlign: 'middle', color: '#0d9488' }}>add_circle</span>
                  Raise New Ticket
                </h5>
              ...
              <form onSubmit={handleCreate}>
                <div className="modal-body" style={{ padding: '1.5rem' }}>
                  <div className="mb-3">
                    ...
                    <input type="text" className="form-control" placeholder="Brief title for your issue" value={form.subject} onChange={e => setForm({ ...form, subject: e.target.value })} required style={{ borderRadius: '10px' }} />
                  </div>
                  ...
                  <div className="mb-0">
                    ...
                    <textarea className="form-control" rows="4" placeholder="Describe your issue in detail..." value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} required style={{ borderRadius: '10px' }}></textarea>
                  </div>
                </div>
                  ...
                  <div className="modal-header" style={{ borderBottom: '1px solid var(--border)', padding: '1.25rem 1.5rem' }}>
                    <div>
                      ...
                      <div className="d-flex gap-2 align-items-center" style={{ fontSize: '0.8125rem' }}>
                        <span style={{ color: '#0d9488', fontWeight: 700 }}>{selectedTicket.ticketNumber}</span>
                  ...
                  <div className="modal-body" style={{ padding: '1.5rem', maxHeight: '60vh', overflowY: 'auto' }}>
                    ...
                    <div className="mb-4 p-3" style={{ background: '#f8f9fa', borderRadius: '12px', border: '1px solid var(--border)' }}>
                      <div className="d-flex justify-content-between align-items-center mb-2">
                        <span style={{ fontWeight: 700, fontSize: '0.875rem' }}>{selectedTicket.raisedBy?.name || 'You'}</span>
